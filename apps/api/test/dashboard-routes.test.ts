...
it("returns a single dashboard read model with explicit no-goal fields", async () => {
  ...
  try {
    const cookie = await bootstrapSession(`http://127.0.0.1:${runtime.port}`);
    const response = await fetch(`http://127.0.0.1:${runtime.port}/api/v1/dashboard/2026-09-09`, { headers: withCookie(cookie) });
...
it("rejects an impossible dashboard date with the common error envelope", async () => {
  ...
  try {
    const cookie = await bootstrapSession(`http://127.0.0.1:${runtime.port}`);
    const response = await fetch(`http://127.0.0.1:${runtime.port}/api/v1/dashboard/2026-02-30`, { headers: withCookie(cookie) });
