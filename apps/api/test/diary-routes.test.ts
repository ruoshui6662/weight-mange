...
it("exposes diary writes with nested validation and optimistic conflict envelopes", async () => {
  ...
  try {
    ...
    const food = await fetch(`${base}/api/v1/foods/custom`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ name: "路由豆浆", nutrients: { energyKcal: 30, proteinG: 2 } }) });
    const { data: { id: foodId } } = await food.json() as { data: { id: string } };
    const created = await fetch(`${base}/api/v1/diary/2026-09-09/entries`, { method: "POST", headers: { "content-type": "application/json", "idempotency-key": "route-entry" }, body: JSON.stringify({ mealSlotId: "breakfast", foodId, amount: 50, unit: "g", source: "manual" }) });
