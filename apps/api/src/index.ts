  ...
  try {
    ...
    const server = createServer(async (request, response) => {
      ...
      try {
        ...
        if (request.method === "GET" && url.pathname === "/api/v1/analytics/tdee") { const from = url.searchParams.get("from") ?? ""; const to = url.searchParams.get("to") ?? ""; writeJson(response, 200, { data: analytics.getAdaptiveTdee({ userId: userId!, from, to, ...(url.searchParams.get("asOfDate") ? { asOfDate: url.searchParams.get("asOfDate")! } : {}) }) }); return; }
        if (bodyWeightMatch && request.method === "POST" && bodyWeightMatch[1] === undefined) { const input = await readJson(request); writeJson(response, 201, { data: body.createWeight({ userId: userId!, ...(typeof input.measuredAt === "string" ? { measuredAt: input.measuredAt } : {}), ...(typeof input.localDate === "string" ? { localDate: input.localDate } : {}), weightKg: input.weightKg as number, ...(typeof input.source === "string" ? { source: input.source as "manual" | "import" } : {}), ...(input.note === null || typeof input.note === "string" ? { note: input.note } : {}) }) }); return; }
