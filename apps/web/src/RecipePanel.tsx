...
export function RecipePanel(props: RecipePanelProps): ReactElement {
  ...
  if (mode === "detail" && recipe) return <section className="recipe-page"><div className="recipe-workspace recipe-detail-workspace"><Surface className="recipe-zone recipe-detail-zone" data-recipe-zone="detail"><div className="recipe-page-heading"><div><span className="dg-eyebrow">RECIPE DETAIL</span><h2>{recipe.name}</h2><p className="muted">原料：{recipe.ingredients.map((item) => `${item.nameSnapshot} ${item.inputAmount}${item.inputUnit}`).join("、")}</p></div><span className="status-chip">计算版本 {recipe.calcVersion}</span></div><div className="entry-actions recipe-detail-actions"><Button variant="secondary" disabled={busy !== null} onClick={() => beginEdit(recipe)}>编辑菜谱</Button><Button variant="secondary" disabled={busy !== null} onClick={() => void copyRecipe()}>复制菜谱</Button><Button variant="secondary" disabled={busy !== null} onClick={() => void refreshRecipe()}>刷新原料</Button><Button variant="destructive" disabled={busy !== null} onClick={() => void deleteRecipe()}>删除菜谱</Button></div><RecipeNutrientResults total={recipe.total} per100g={recipe.per100g} perServing={recipe.perServing} warnings={recipe.warnings} /><div className="recipe-zone recipe-diary-zone" data-recipe-zone="diary"><div className="recipe-subheading"><div><span className="dg-eyebrow">QUICK LOG</span><h3>加入日记</h3></div><span className="muted">记录时保存当前快照</span></div><form className="add-form" onSubmit={(event) => void addToDiary(event)}><div className="recipe-field-grid"><label className="field"><span>餐次</span><select aria-label="餐次" value={diaryMeal} onChange={(event) => setDiaryMeal(event.target.value)}><option value="breakfast">早餐</option><option value="lunch">午餐</option><option value="dinner">晚餐</option><option value="snack">加餐</option></select></label><label className="field"><span>用量（g）</span><input aria-label="日记用量" type="number" min="1" step="0.1" value={diaryAmount} onChange={(event) => setDiaryAmount(event.target.value)} /></label></div><Button type="submit" variant="secondary" disabled={busy !== null} busy={busy === "diary"}>加入日记</Button></form></div>{error ? <p className="error" role="alert">{error}</p> : null}<Button type="button" variant="tertiary" onClick={() => { setRecipe(null); setMode("list"); }}>返回菜谱列表</Button></Surface></div></section>;
