...
export function RecipePanel(props: RecipePanelProps): ReactElement {
  ...
  if (mode === "editor") return <section className="card add-card">{header}<form className="form" onSubmit={(event) => void save(event)}><label className="field"><span>菜谱名称</span><input aria-label="菜谱名称" value={draft.name} disabled={draftLocked} onChange={(event) => setDraft({ ...draft, name: event.target.value })} /></label><label className="field"><span>成品重量（g，可选）</span><input aria-label="成品重量" type="number" min="0" step="0.1" value={draft.cookedWeightG} disabled={draftLocked} onChange={(event) => setDraft({ ...draft, cookedWeightG: event.target.value })} /></label><label className="field"><span>份数（可选）</span><input aria-label="份数" type="number" min="0" step="0.1" value={draft.servingCount} disabled={draftLocked} onChange={(event) => setDraft({ ...draft, servingCount: event.target.value })} /></label><h3>原料</h3>{draft.ingredients.map((row) => <div className="card" key={row.key}><label className="field"><span>原料名称</span><input aria-label="原料搜索" value={searches[row.key] ?? row.name} disabled={draftLocked} onChange={(event) => setSearches((current) => ({ ...current, [row.key]: event.target.value }))} /></label><div className="entry-actions"><button type="button" className="soft-button" disabled={draftLocked || searchPending[row.key] === true} onClick={(event) => void searchFood(event, row)}>搜索原料</button>{draft.ingredients.length > 1 ? <button type="button" className="soft-button" disabled={draftLocked} onClick={() => setDraft((current) => ({ ...current, ingredients: current.ingredients.filter((item) => item.key !== row.key) }))}>删除原料</button> : null}</div>{(foodResults[row.key] ?? []).map((food) => <button type="button" className="food-result" key={food.id} aria-label="选择原料" disabled={draftLocked} onClick={() => chooseFood(row, food)}><span>{food.name}</span><small>{food.summary.energyKcal ?? "—"} kcal / 100g</small></button>)}<RecipeFoodSearchStatus searched={foodResults[row.key] !== undefined} resultCount={(foodResults[row.key] ?? []).length} />{searchError[row.key] ? <p className="error" role="alert">{searchError[row.key]}</p> : null}<label className="field"><span>用量（{row.originalUnit === "g" || row.edited ? "g" : "克等值"}）</span><input aria-label="原料用量" type="number" min="0" step="0.1" value={row.amount} disabled={draftLocked} onChange={(event) => updateRow(row.key, { amount: event.target.value })} /></label><p className="muted">已选：{row.name || "尚未选择"}</p></div>)}<button type="button" className="soft-button" disabled={draftLocked} onClick={() => setDraft((current) => ({ ...current, ingredients: [...current.ingredients, emptyRow()] }))}>添加原料</button>{error ? <p className="error" role="alert">{error}</p> : null}{conflictRecipeId ? <button type="button" className="soft-button" onClick={() => void reloadAfterConflict()} disabled={busy !== null}>重新加载最新菜谱（保留当前草稿）</button> : null}<button type="submit" className="primary" disabled={busy !== null}>{busy === "save" ? "保存中…" : "保存菜谱"}</button><button type="button" className="soft-button" disabled={draftLocked} onClick={() => { setMode(recipe ? "detail" : "list"); setError(""); }}>{draftLocked ? "取消（保存中…）" : "取消"}</button></form></section>;
