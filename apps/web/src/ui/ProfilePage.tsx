      ...
      <div className="profile-settings-main">
        <Surface className="profile-section">
          ...
          <div className="profile-setting-list">
            <SettingRow label="账户称呼" value={props.profile?.displayName ?? "未设置"} />
            <SettingRow label="时区" value={props.profile?.timezone ?? "未设置"} />
          </div>
        </Surface>
        <Surface className="profile-section">
        ...
        <Surface className="profile-section">
          ...
          <div className="profile-setting-list">
            ...
            <SettingRow label="备份位置" value="由服务端管理" />
          </div>
...
function SettingRow(props: { label: string; value: string; action?: string; onAction?: (() => void) | undefined }) {
  return <div className="profile-setting-row"><div><strong>{props.label}</strong><span>{props.value}</span></div>{props.action ? <Button type="button" variant="tertiary" onClick={props.onAction}>{props.action}</Button> : null}</div>;
}
