...
export function AnalyticsPage(props: AnalyticsPageProps) {
  return <div className="dg-page analytics-page" data-analytics-layout="evidence-first">
    <div className="analytics-page-main">
      ...
      {!props.loading && !props.error && props.overview ? <FactSummary overview={props.overview} /> : null}
      {!props.loading && !props.error ? <EstimateSummary tdee={props.tdee} /> : null}
    </div>
...
function EstimateSummary({ tdee }: { tdee: TdeeEstimate | null }) {
  const estimated = tdee?.status === "estimated" && tdee.estimatedTdeeKcal !== null;
  return <section className="analytics-estimate" aria-labelledby="analytics-estimate-title"><div className="analytics-section-heading"><div><span className="dg-eyebrow">SERVER ESTIMATE</span><h2 id="analytics-estimate-title">服务端估算 · Adaptive TDEE</h2><p className="dg-muted">这是服务端基于现有数据返回的估算，不是客户端重新计算的目标。</p></div><span className="dg-status-chip">{estimated ? "估算" : "数据不足"}</span></div>{estimated ? <div className="analytics-estimate-result"><strong>{number.format(Math.round(tdee.estimatedTdeeKcal!))} kcal</strong><span>估算维持热量</span><small>方法 {tdee.methodVersion} · 置信度 {number.format(Math.round(tdee.confidence * 100))}%</small></div> : <StatusMessage kind="empty" title="数据不足" description={tdee?.reason ? `当前周期暂不能生成 Adaptive TDEE（${tdee.reason}）。继续记录真实饮食和体重后再查看。` : "当前周期暂不能生成 Adaptive TDEE。不会伪造估算。"} />}</section>;
}
