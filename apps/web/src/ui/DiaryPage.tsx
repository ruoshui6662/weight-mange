  ...
  return <div className="dg-page diary-page" data-diary-layout="workspace">
    <div className="diary-workspace">
      <Surface className="diary-search-panel">
        ...
        <SearchState {...props} />
        {props.searchStatus === "success" ? <div className="diary-results" aria-label="食物搜索结果">{props.results.map((food) => <button type="button" className={`diary-result-row ${props.selected === food.id ? "selected" : ""}`} data-food-result={food.id} aria-label={`选择${food.name}`} aria-pressed={props.selected === food.id} key={food.id} onClick={() => props.setSelected(food.id)}><span><strong>{food.name}</strong><small>本地目录 · 每 100g</small></span><span className="diary-result-kcal">{food.summary.energyKcal ?? "—"} kcal</span></button>)}</div> : null}
...
function QuantityConfirmation(props: DiaryPageProps) {
  ...
  return <form className="diary-confirmation" onSubmit={(event) => void props.onAddEntry(event)}>
    ...
    <Button type="submit" variant="primary" busy={props.busy}>加入饮食记录</Button>
  </form>;
}
...
function DiaryEntry(props: DiaryPageProps & { entry: DiaryEntryForPage }) {
  ...
  return <div className="diary-entry"><div className="diary-entry-main"><strong>{props.entry.displayName}</strong><span>{props.entry.amount}{props.entry.unit}</span></div><div className="diary-entry-actions"><Button variant="tertiary" type="button" onClick={() => props.onEdit(props.entry)}>编辑</Button><Button variant="destructive" type="button" busy={props.busyEntry === props.entry.id} onClick={() => void props.onDelete(props.entry)}>删除</Button></div>{editing ? <form className="diary-edit-form" onSubmit={(event) => { event.preventDefault(); void props.onSave(props.editingEntry!); }}><label><span>份量（g）</span><input type="number" min="1" value={props.editingEntry!.amount} onChange={(event) => props.onEdit({ ...props.editingEntry!, amount: Number(event.target.value) })} /></label><label><span>餐次</span><select aria-label={`编辑餐次-${props.entry.displayName}`} value={props.editingEntry!.mealSlotId} onChange={(event) => props.onEdit({ ...props.editingEntry!, mealSlotId: event.target.value })}><option value="breakfast">早餐</option><option value="lunch">午餐</option><option value="dinner">晚餐</option><option value="snack">加餐</option></select></label><div><Button type="submit" variant="primary" busy={props.busyEntry === props.entry.id}>保存修改</Button><Button type="button" variant="secondary" onClick={props.onCancelEdit}>取消</Button></div></form> : null}</div>;
}
