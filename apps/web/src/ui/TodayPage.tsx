...
function DiaryEntry(props: TodayPageProps & { entry: EditableEntry }) {
  ...
  return <div className="today-entry"><div><strong>{props.entry.displayName}</strong><span className="today-muted">{props.entry.amount}{props.entry.unit}</span></div><div className="today-entry-actions"><Button variant="tertiary" onClick={() => props.onEdit(props.entry)}>编辑</Button><Button variant="destructive" onClick={() => void props.onDelete(props.entry)} disabled={props.busyEntry === props.entry.id}>删除</Button></div>{isEditing ? <form className="today-entry-edit" onSubmit={(event) => { event.preventDefault(); void props.onSave(props.editingEntry!); }}><label className="field"><span>份量（{props.entry.unit}）</span><input type="number" min="1" step="0.1" value={props.editingEntry?.amount ?? props.entry.amount} onChange={(event) => props.onEdit({ ...props.editingEntry!, amount: Number(event.target.value) })} /></label><div className="today-entry-actions"><Button type="submit" variant="primary" busy={props.busyEntry === props.entry.id}>保存修改</Button><Button type="button" variant="secondary" onClick={props.onCancelEdit}>取消</Button></div></form> : null}</div>;
}
