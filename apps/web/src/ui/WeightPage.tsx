  ...
  return <div className="dg-page weight-page" data-weight-layout="trend-first">
    <div className="weight-page-main">
      <Surface className="weight-hero">
        <div><span className="dg-eyebrow">BODY · CURRENT WEIGHT</span><h2>当前体重</h2>{latest ? <><strong className="weight-hero-value">{latest.weightKg.toFixed(1)}<small> kg</small></strong><p className="dg-muted">记录于 {latest.localDate} · {latest.source === "manual" ? "手动记录" : latest.source}</p></> : <StatusMessage kind="empty" title="尚无足够记录" description="记录一次真实体重后，这里会显示当前值和趋势。" />}</div>
        <div className="weight-hero-badge" aria-label={trendLatest === undefined ? "趋势暂不可用" : `趋势体重 ${trendLatest.toFixed(1)} kg`}>{trendLatest === undefined ? "—" : `${trendLatest.toFixed(1)} kg`}<span>趋势值</span></div>
      </Surface>
