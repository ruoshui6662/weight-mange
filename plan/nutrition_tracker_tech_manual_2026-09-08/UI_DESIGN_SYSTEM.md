...
**今日饮食记录卡：** 使用“今日饮食记录”标题、说明文字和 Primary“＋ 记录饮食”总入口；早餐、午餐、晚餐、加餐各自显示图标/名称、当餐 kcal 和 Secondary“＋ 添加”按钮，复制昨日动作作为 Tertiary 辅助操作。四餐均无记录时显示轻量虚线空状态和“记录饮食”按钮；任一餐有记录后隐藏空状态。点击总入口默认进入早餐，点击分餐入口进入对应餐次；两者都滚动并聚焦同一搜索面板，搜索、选择食物、确认克数后使用“加入早餐/午餐/晚餐/加餐”动态按钮提交。添加成功刷新日记快照与今日汇总，失败保留用户输入。
