...
M3-012 会话与读取规则：刷新恢复、主动登录及首次设置完成共用资料/目标检查，读取资料时区后再请求当天预算和日记。首次读取未完成时显示准备状态，不把尚未加载的数据当作空记录或零摄入；连接/服务故障显示明确错误及“重新加载”，只重试读取，不要求重新输入密码。未认证或受保护读取返回 401 才进入登录。切页、切换分析周期和退出后，旧请求不得更新当前视图或错误状态。已有成功快照的后台刷新失败不清空快照；写入结果确认和重试幂等另由 M3-013 实施。
...
1. PageHeader：日期切换、问候语；饮食写入由餐次卡片的添加按钮承担；
2. Hero：单张“今日热量”摘要卡，上部以“已摄入—还可以吃热量环—今日目标”建立主信息层级，并明确完成度；
3. Macro：合并在 Hero 底部，碳水、蛋白质、脂肪保持三个等宽信息列，各自显示当前值/目标、固定颜色进度条和比例；不再使用三张独立卡；
4. Meal grid：早餐/午餐两列，晚餐/加餐两列；
5. Weekly overview：一张轻量 7 日摄入趋势图；
6. ContextRail：今日数据质量提示；不重复放置饮食写入入口。
...
**今日饮食记录卡：** 使用“今日饮食记录”标题和说明文字；早餐、午餐、晚餐、加餐各自显示图标/名称、当餐 kcal 和 Secondary“＋ 添加”按钮，餐次添加按钮是唯一写入入口。整天复制昨日作为唯一批量辅助操作，位于卡片头部；餐次卡不提供重复的分餐复制入口。四餐均无记录时显示轻量虚线空状态提示，文案指向对应餐次的“添加”按钮，不再暗示已移除的下方快捷区，也不放置重复的“记录饮食”按钮。点击任一餐次添加按钮打开共享记录弹窗，默认选中对应餐次并自动聚焦搜索框；搜索、选择食物、确认克数后使用“加入早餐/午餐/晚餐/加餐”动态按钮提交。添加成功关闭弹窗并刷新日记快照与今日汇总，失败保留弹窗和用户输入；弹窗打开时背景容器设置 inert/aria-hidden，Tab 与 Shift+Tab 在弹窗内循环；Esc、关闭按钮可安全退出并将焦点恢复到触发按钮。

**按钮：** 餐次卡片使用 Secondary“添加食物”；弹窗内使用 Primary“加入早餐/加入午餐…”；其余页面按任务使用“记录体重”“添加菜谱”和 Tertiary“查看详情”。

**空状态：** 餐次显示“还没有记录 · 点击‘添加’开始记录”，操作指向同一餐次卡片右侧的“添加食物”按钮；不显示大型插画。

**移动端：** 热量主信息使用“已摄入—热量环—今日目标”三列紧凑重排；宏量信息位于同一卡片底部并始终保持三列，使用更小间距和字号但不改为三张纵向卡。餐次变为单列；周图移到餐次之后；点击餐次添加后使用底部 Sheet 形态的共享弹窗，搜索框自动聚焦，确认按钮保持可见；弹窗焦点隔离规则与桌面一致；底部导航不遮最后一个餐次。最小 360px 视口不得产生横向滚动，数值使用不换行的制表数字。
