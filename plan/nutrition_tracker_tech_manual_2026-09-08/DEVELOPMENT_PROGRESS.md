...
| 下一步 | 读取运动产品/技术规范，定义运动记录、MET 计算、历史 snapshot 边界，并按 DOC-005 的 UI 壳层拆分实施任务 |
| 当前阻塞 | 无环境阻塞；本机 Docker CLI 仍缺失，仅影响容器实测，不影响 CI buildx |
| 业务代码 | M1-001 至 M1-009、M2-001 至 M2-006、M3-001 至 M3-003 已完成；DOC-005 UI 规范已完成，下一项为 M3-004 |
| Git | 本地 `main` 已包含 DOC-005 文档提交，工作区干净；远程发布不属于本任务 |
...
| M3 | 菜谱、运动与预算策略 | `IN_PROGRESS` | 3/6 | M3-003 与 DOC-005 已完成；下一项实施 M3-004 MET/运动计算 |
