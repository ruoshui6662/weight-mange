...
> 2026-09-12 审核建议：优先执行 [功能修正队列](./FUNCTION_AUDIT_AND_REMEDIATION.md#4-修改任务与顺序) 的 M3-012～016，保障会话、写入、日期、目标和趋势可靠性，再补齐交互及继续原运动计划。原任务范围保留；状态以 DEVELOPMENT_PROGRESS.md 为准，本条不表示已实施。
...
- **M3-010 今日页体重趋势摘要：** 在今日页展示最近一周真实体重趋势；有记录才显示，无记录隐藏；复用服务端趋势值，不新增 API 或数据库迁移。
- **M3-011 今日饮食记录功能区：** 将今日页餐次摘要重构为可操作的饮食记录卡；提供四餐添加入口、空状态、目标餐次联动和搜索写入闭环；复用既有日记 API 与营养快照，不新增 API 或数据库迁移。
